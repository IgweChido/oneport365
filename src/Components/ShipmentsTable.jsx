...
function ShipmentsTable({
  ...
  useEffect(() => {
    const filterData = () => {
      if (input?.length > 0) {
        console.log(">>>>0");
        const filtD = data?.filter(
          (ship, id) =>
            (ship.origin_port_code?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (ship.origin_port_city?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (ship.origin_port_country?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (ship.destination_port_city?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (
              ship.destination_port_country?.toString().toLowerCase() + ""
            ).indexOf(input.toLowerCase()) >
              -1 ===
              true ||
            (ship.destination_port_code?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (ship.delivery_location?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (ship.pickup_location?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
            (ship.state?.toString().toLowerCase() + "").indexOf(
              input.toLowerCase()
            ) >
              -1 ===
              true ||
  ...
  useEffect(() => {
    if (typeButton?.length > 0) {
      const filtD = data?.filter((ship) => ship.shipping_type === typeButton);
                  ...
                  <tr key={ship._id}>
                    <td className="">
                      <div className=" name flex items-center space-x-3 py-[20px]">
                        ...
                        <p className="font-semibold text-[16px] 2xl:text-[18px] pr-[30px]">
                    ...
                    <td className="font-semibold text-[16px] 2xl:text-[18px] pr-[20px]">
                      {ship.shipment_pickup_date.slice(0, 10)}
                    </td>
                    <td className="font-semibold text-[16px] 2xl:text-[18px] max-w-[50px]">
                      <p>{ship._id.slice(0, 10)}...</p>
                    </td>
                    <td className="min-w-[230px]">
                      <div className="flex flex-1 justify-end items-center">
                        <Link
                          ...
                          className="py-3.5 px-5 text-sm rounded-md bg-main-green ml-[19px] mr-[18px] text-white"
                        >
                          View Details
                        </Link>
                      </div>
