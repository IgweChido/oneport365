...
function MainPage({ setFixed }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (
        location.pathname === "/" ||
        location.pathname === `/shipment/${customerr.id_cust.id}` ||
        location.pathname === "/shipmentdetail"
      ) {
        moveToTop();
      }
    }, 1000);
  ...
  return (
    <div className="flex-1  relative  overflow-auto w-full  " ref={ref}>
      ...
      <div className="py-[150px] laptop:py-[200px] px-[16px] sm:px-[40px] lg:px-[50px] 2xl:px-[70px] ">
        {pSlice && <PhoneNavigation setFixed={setFixed} />}
        <div className=" max-w-[1280px] mx-auto   ">
          <Routes>
            ...
            <Route
              path="/shipmentdetail"
              element={<ShipmentDetails setFixed={setFixed} />}
            />
          </Routes>
        </div>
      </div>
